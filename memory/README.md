...
| **M07 — Quiz** | Agente C | claude-sonnet-4-6 | 📋 Pendente | — |
...
1. **Identifique a aula:**
   ...
   - Selecione o número da **Aula** (1 a 8, ou "todas" para separador automático)
...
4. **Modo de testes:**
   - Botão "⚡ Preencher para Testes" auto-preenche com material de teste
   - Botão "🧹 LIMPAR TESTES" restaura estado original
...
Exemplo: `cursos/administracao/FUN-fundamentos-de-administracao/aulas/01/`
...
PDF multi-aula (opção "Todas")
  → 02-separar-aulas.py → detecta "Conversa Inicial" + "Aula 01-08"
  → Cria pastas aulas/00/, aulas/01/, etc.
```
...
# MÓDULO 01b — SEPARADOR DE AULAS ✅

Separa PDF único contendo múltiplas aulas em pastas individuais.

**Detecta:**
- "Conversa Inicial" → aula `00` (ID: `{CODIGO}-00`)
- "Aula 01", "Aula 02", etc. → aulas `01`, `02`, ...

**Como usar:**
- Interface: selecione "Todas (CI + 01-08)" no combo de aulas
- CLI: `python3 scripts/02-separar-aulas.py --codigo FUN --disciplina "..." --pdf ...`
...
| `modulo02/referencias.py` | Extrai referências do markdown quando Agente E não as grava |
...
O M02 valida estritamente antes de gerar score:
...
### Melhorias no Laudo (pós-correção)
...
- Fallback `aula_id` corrigido para carregamento automático
...
| `servidor.py` `_executar_m03_via_api()` | Execução via API Anthropic direta |
...
02_markdown/{ID}.md (aprovado M02)
        ↓
   Agente A (Sonnet 4.6) via API Anthropic
        ↓
03_reformulado/
...
1. Acesse `/m03-preview?curso=...&codigo=...&disciplina=...&aula=...`
2. Clique em **"Criar Texto Display"** (botão no topo)
3. Aguarde processamento (spinner com mensagem)
4. Revise o resultado em `display.html`
5. Se validador OK → clique **"Aprovar e seguir para imagens"**
...
**Correções recentes:**
- Regex `:::([\w-]+)` para callouts com hífen (`conceito-chave`)
- Word boundary `\b` em palavras proibidas (evita falso positivo em "unidade", "uniforme")
...
- Carregamento automático via URL: `/modulo03/display.html?params`
...
│       └── texto-display.md        # M03: Prompt do Agente A
├── dados/
│   └── catalogo.csv                # Catálogo de cursos/disciplinas (piloto)
├── docs/
│   ├── INSTRUCOES_CHAT.md
│   ├── PROMPT_MESTRE.md
│   └── voz-unigran.md              # Voz e tom editorial
├── interface/
│   ├── index.html                  # Hub de entrada (:5050)
│   └── m03-preview.html            # Preview M03 (passo-a-passo)
├── modulo02/
...
│   └── 04-agente-a.py              # M03: Texto Display (CLI)
├── skills/
...
├── testes/                         # Material de teste
│   └── adm_fund_aula01.pdf         # CI (p.6) + Aula 01 (p.7–23)
├── CLAUDE.md                       # Instruções do projeto
├── HANDOFF.md                      # Estado da sessão (atualizar ao final)
...
|--------|----------------|----------|
| **M01** | Extração Word/PDF → Markdown + imagens | `01-processar-entrada.py` |
...
| **M03** | Texto Display (Agente A) | `servidor.py` (API direta), `validador.py`, `display.html` |

### 🔧 Interface da esteira

- Hub de entrada com catálogo de disciplinas (`interface/index.html`)
- Modo de testes com auto-preenchimento
- Indicador de status da IA (🟢 online / 🟡 fallback / 🔴 offline)
- Laudo visual com carregamento automático (`modulo02/laudo.html`)
- Preview M03 com fluxo passo-a-passo (`interface/m03-preview.html`)
- Tela de revisão display com validador (`modulo03/display.html`)
- API REST (`servidor.py`): `/api/catalogo`, `/api/processar`, `/api/score`, `/api/m03-executar`, `/api/m03-check`, `/api/ia-status`
...
|----|-----------|------------|
| **P2** | Sumário do PDF vaza no markdown (intervalo de páginas inclui página de sumário) | Média |
| **P3** | PDF multi-aula enviado como aula individual não é detectado | Média |
| **P4** | `display.html` usa validação inline (cliente), não a do servidor | Baixa |
